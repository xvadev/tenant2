...
if($id > 0) {
	...
	?>
		...
		<div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                  <div class="x_title">
                    <h2>Manage Tenants <small></small></h2>
                  ...
                  <div class="x_content" style="display: block;">
						...
						<?php
					$ann = Yii::$app->db->createCommand('SELECT tenant_id, first_name, middle_name, last_name, company, email, phone, property_id FROM tenant WHERE user_id='.$id.'')->queryAll();
					
					?>
					<form method="GET" action="<?php echo $_SERVER['PHP_SELF'];?>">
					<a class="btn btn-success" href="index.php?r=site/page&view=tenant-add"  style="border-radius: 4px;height: 37px!important;width: 130px;font-size: 18px;margin-right: 5px;margin-bottom: 13px;line-height: 1.5;">Create New</a>
					<button class="btn btn-success" name="rent" style="border-radius: 4px;height: 37px!important;width: 120px;font-size: 18px;margin-right: 5px; float:right;">Rent</button>
					...
					<input type="hidden" name="view" value="manage-tenants">
							...
							<th>Rented Property</th>
								...
								<tbody>
								  ...
								  <?php
									...
									<td class=" "><input type="checkbox" name="check_list[]" value="<?php echo $value['tenant_id']; ?>"></td>
									<td class=" "><?php echo $value['tenant_id']; ?></td>
									<td class=" "><?php echo $value['last_name'].', '.$value['first_name'].' '.$value['middle_name'];?></td>
									<td class=" "><?php echo $value['email']; ?></td>
									<td class=" "><?php echo $value['phone']; ?></td>
									<td class=" last"><?php echo $property; ?>
									</td>
									</tr>
									<?php
									}
		...
		<?php
			...
			if($pos == 3) {
				...
				<h1>Manage Tenants</h1>
						...
						<input type="hidden" name="r" value="site/page">
						<input type="hidden" name="_csrf" value="<?=Yii::$app->request->getCsrfToken()?>" />
						<input type="hidden" name="view" value="manage-tenant">
						...
						?>
							  ...
							  <tr>
								<td><input type="checkbox" name="check_list[]" value="<?php echo $value['tenant_id']; ?>"></td>
					...
					<button name="rent" style="border-radius: 4px;height: 37px!important;width: 120px;font-size: 18px;margin-right: 5px; float:right;">Rent</button>
	...
	{	
		foreach($_GET['check_list'] as $selected) {
			...
			$sql 	 = $command->delete('tenant', 'tenant_id=:id', array(
						':id'		=> $selected
						));
		}
		if(!$sql->execute())
		{
			Yii::$app->response->redirect("index.php?r=site/page&view=manage-tenants&msg=failed")->send();
			//header("Location: index.php?r=site/page&view=manage-ann&msg=failed");
		}
		else
		{
			Yii::$app->response->redirect("index.php?r=site/page&view=manage-tenants&msg=success")->send();
		} 
	}
	if(isset($_GET['check_list']) && isset($_GET['edit'])) 
	{	
		foreach($_GET['check_list'] as $selected) {
			Yii::$app->response->redirect("index.php?r=site/page&view=tenant-edit&check_list=".$selected."")->send();
			//header("Location: index.php?r=site/page&view=edit-announcement&check_list=".$selected."");
		}
	}
	if(isset($_GET['check_list']) && isset($_GET['rent'])) 
	{	
		foreach($_GET['check_list'] as $selected) {
			Yii::$app->response->redirect("index.php?r=site/page&view=tenant-rent&check_list=".$selected."")->send();
		}
	}
}
else {
