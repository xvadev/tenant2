...
$this->title = 'Edit Building Manager';
...
if(isset($_GET['fname']) && !empty($_GET['fname']) )
{
	...
	$sql 	 = $command->update('bmanager', array(
					 'user_id'		=> $id,
					 ...
					 'bmanager_id=:id', array(':id'	=> $_GET['t_pcid'])
					 );

	if(!$sql)
	{
		header("Location: index.php?r=site/page&view=edit-bmanager&msg=failed");
	}
	else
	{
		header("Location: index.php?r=site/page&view=edit-bmanager&msg=success");
	}

}
if(isset($_GET['check_list']) && !empty($_GET['check_list']) ) {
	$ck = $_GET['check_list'];
	 $ann = Yii::$app->db->createCommand('SELECT bmanager_id, first_name, middle_name, last_name, company, email, phone, property_id FROM bmanager WHERE bmanager_id='.$ck.'')->queryAll();
					
	foreach ($ann as $key2 => $value2)
						{
							$tenant_id = $value2['bmanager_id'];
...
<div class="container">
	<div class="row">
		...
		<?php
				if(isset($_GET['msg']))
				{
					...
					{
						?>
						<p style="color:#D8000C; background:#FFBABA; padding:4px 6px 4px 6px;margin-left: 53px;">Failed to Update Building Manager!</p>
						<?php
					}
				}
			?>
	...
	<div class="row-fluid">
           <div class="span12">
		   <a href="/tenant/index.php?r=site/page&view=manage-bmanager"><button class="btn-primary">Go back to Manage Building Manager</button></a>
	...
	<input type="hidden" name="view" value="edit-bmanager">
	...
	<div class="row">
		...
		<div class="row-fluid">
			<button class="btn btn-large btn-primary" style="height: 46px!important; width:120px; font-size: 18px;float:right;">Submit</button>
		</div>
	</form>
	</div>
	</div>
...
<?php
	...
	<div class="row">
		<div class="col-md-12">
			...
			?>
			<a href="/tenant/index.php?r=site/page&view=manage-bmanager"><button class="btn-primary">Go back to Manage owners</button></a>
		</div>
	</div>
	</div>
	</div>
